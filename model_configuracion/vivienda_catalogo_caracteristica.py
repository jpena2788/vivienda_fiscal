...
class CatalogoCaracteristica(models.Model):
    _name = 'vivienda.catalogo_caracteristica'
    ...
    caracteristica_valor_ids = fields.One2many(string='Valor Característica', comodel_name='vivienda.caracteristica_valor', inverse_name='caracteristica_id', )
    active = fields.Boolean(string='Activo/Inactivo', default='True' )
    
    _sql_constraints = [('name_unique', 'UNIQUE(name)', "Nombre del atributo debe ser unico en cada categoría!!"),] 
...
class ValorCaracteristica(models.Model):
    _name = "vivienda.caracteristica_valor"   
    ...
    caracteristica_id = fields.Many2one('vivienda.catalogo_caracteristica', string="Valor", ondelete='restrict', index=True,)
