...
class Grupos(models.Model):    
    ...
    def activar_perfiles_hospedaje(self):        
        self.action_hospedaje()
    ...
    def action_hospedaje(self):
        ...
        view = self.env.ref('hospedaje.user_groups_hospedaje_inherit', raise_if_not_found=False)
        ...
        if self._context.get('install_filename') or self._context.get(MODULE_UNINSTALL_FLAG):
            ...
            for app, kind, gs, category_name in sorted_tuples:  # we process the user type first
                ...
                # if kind == 'selection' and app in [self.env.ref("hospedaje.module_hospedaje")]:
                ...
                if kind != 'selection' and app in [self.env.ref("hospedaje.module_hospedaje_especificos")]:
